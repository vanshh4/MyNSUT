...
describe("completeOnboarding", () => {
  beforeEach(() => {
    ...
    prismaMock.$transaction.mockImplementation(async (callback: (tx: object) => Promise<unknown>) =>
      callback({})
    );
  });
  ...
  it("creates the student in one transaction after resolving an existing class", async () => {
    await expect(
      completeOnboarding("user-id", { umsRollNumber: "2023uit3324", section: "2" })
    ...
    expect(resolveExistingClass).toHaveBeenCalledWith(expect.anything(), {
      admissionYear: 2023,
      branchCode: "UIT",
      section: "2",
    });
    expect(studentsRepository.createStudentAndCompleteOnboarding).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ userId: "user-id", classId: "class-id" })
    );
  });
  ...
  it("rejects a duplicate roll number", async () => {
    ...
    await expect(
      completeOnboarding("user-id", { umsRollNumber: "2023UIT3324", section: "2" })
    ).rejects.toMatchObject({ code: "ROLL_NUMBER_ALREADY_REGISTERED" });
  });

  it("rejects onboarding when the class does not exist", async () => {
    resolveExistingClass.mockRejectedValue(
      Object.assign(new Error("Class unavailable"), { code: "CLASS_NOT_AVAILABLE" })
    );
    await expect(
      completeOnboarding("user-id", { umsRollNumber: "2023UIT3324", section: "2" })
    ).rejects.toMatchObject({ code: "CLASS_NOT_AVAILABLE" });
  });
});

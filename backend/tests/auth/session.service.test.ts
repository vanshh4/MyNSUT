...
describe("session service", () => {
  ...
  it("revokes the oldest active session before creating a fourth", async () => {
    ...
    expect(repository.revokeSessionsByIds).toHaveBeenCalledWith(
      expect.anything(),
      ["oldest"],
      expect.any(Date)
    );
    expect(repository.createSession).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ userId: "user-id", ipAddress: "127.0.0.1", userAgent: "Vitest" })
    );
  });
  ...
  it("resolves a valid active session", async () => {
    ...
    await expect(resolveSession("raw-token")).resolves.toMatchObject({

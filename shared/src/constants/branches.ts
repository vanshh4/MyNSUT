/** Canonical undergraduate branch codes supported by MyNSUT onboarding. */
export const BRANCHES = {
  ...
  UIC: "Instrumentation and Control",
...
export interface BranchDefinition { code: BranchCode; name: BranchName; }
...
export const BRANCH_OPTIONS = Object.freeze(
  BRANCH_CODES.map((code) => ({ code, name: BRANCHES[code] }))
) as readonly BranchDefinition[];
export function isBranchCode(value: unknown): value is BranchCode {
  return typeof value === "string" && BRANCH_CODES.includes(value as BranchCode);
}
export function getBranchName(code: BranchCode): BranchName { return BRANCHES[code]; }
